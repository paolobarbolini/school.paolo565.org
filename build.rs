...
fn main() {
    ...
    let content = fs::read_to_string(in_path).expect("unable to open static/service-worker.js");
    let content = content.replace("{{last_commit_hash}}", &hash);

    fs::write(out_path, content).expect("unable to write to static/service-worker.build.js");
}
